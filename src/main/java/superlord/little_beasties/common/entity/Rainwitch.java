...
public class Rainwitch extends WaterAnimal implements Bucketable {
	...
	protected void handleAirSupply(int p_30344_) {
	      if (this.isAlive() && (!this.isInWaterOrBubble() || !this.level().isRainingAt(this.blockPosition()))) {
	...
	class StayInWaterGoal extends Goal {
		...
		public void tick() {
			...
			var blockAbove = mob.level().getBlockState(blockPos.above(3));
			var blockBelow = mob.level().getBlockState(blockPos.below(1));
			var amount = amount();
			
			if (mob.level().isRainingAt(mob.blockPosition())) {
				var rainBlockBelow = mob.level().getBlockState(blockPos.below(3));
//				var rainBlockAbove = mob.level().getBlockState(blockPos.above(3));
				if (rainBlockBelow.getFluidState().isEmpty()) {
